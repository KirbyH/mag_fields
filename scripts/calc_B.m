...
% adjust points very close together
WIRE_DIAM = 0.2;  % [m], this is not accurate but we will give it a shot
filter = r < WIRE_DIAM; 
if any(filter)
    dB(filter,:) = dB(filter,:).*r(filter)/WIRE_DIAM; 
end
...
% disp(num2str(r)); 
% if r < WIRE_DIAM
%     B_field = B_field*r/WIRE_DIAM;  % vary linearly 
%     disp('less than wire diam, reduced by ' + num2str(r/WIRE_DIAM)); 
% end
end