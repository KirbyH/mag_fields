...
nPan = round(linspace(8, 100, nTests));  % number of panels
...
for ii = 1:nTests
    % racetrack tests
    geom = coil_racetrack(1, 3, nPan(ii)+1, 2); 
...
figure("name", "Total Forces vs Discretization"); hold on; grid on; 
colororder(Colors); 
yyaxis left 
...
figure("name", "Panel Forces vs Discretization"); 
colororder(Colors(2,:)); 
